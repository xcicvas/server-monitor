...
所有数据（服务器列表、配置）存储在**浏览器 sessionStorage** 中，关闭浏览器标签页后自动清除，完全不上传到任何服务器。
